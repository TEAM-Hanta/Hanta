...
module.exports = class Note {
    ...
    static sendNoteList(user_id) {
        return db.execute('SELECT content,send_date,note_read_type FROM note JOIN note_content ON note.note_type = 0 AND note.user_id = ? AND note_content.id = note.note_content_id', [user_id]);
    }

    // 내가 받은 쪽지 리스트
    static receiveNoteList(user_id) {
        return db.execute('SELECT content,send_date,note_read_type FROM note JOIN note_content ON note.note_type = 1 AND note.user_id = ? AND note_content.id = note.note_content_id', [user_id]);
    }
